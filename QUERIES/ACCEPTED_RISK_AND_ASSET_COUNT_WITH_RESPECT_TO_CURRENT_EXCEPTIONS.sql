...
total_accepted_risk AS (
   SELECT
      SUM(dv.riskscore :: INTEGER) AS total_accepted_risk,
      favie.vulnerability_exception_id,
      COUNT(DISTINCT favie.asset_id) AS asset_count
   FROM
      fact_asset_vulnerability_instance_excluded favie
      JOIN dim_vulnerability dv ON (dv.vulnerability_id = favie.vulnerability_id)
   GROUP BY
      favie.vulnerability_exception_id
)
SELECT
   DISTINCT exceptionscope,
   COALESCE(el.additional_comments, '') as additional_comments,
   el.submitted_date,
   el.submitted_by,
   el.review_date,
   el.reviewed_by,
   ...
   el.reason,
   el.title,
   (el.riskscore :: Integer),
   (el.cvss_v2_score :: Integer),
   COALESCE(total_accepted_risk, 0) AS "Total Accepted Risk",
   tar.asset_count AS "Asset Count"
FROM
   exceptions_list el
...
WHERE
   ...
   or el.expiration_date is null
ORDER BY
   "Total Accepted Risk" DESC