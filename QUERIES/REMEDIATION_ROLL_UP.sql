...
WITH superceding_solution_summaries AS (
   SELECT
      dshs.solution_id AS superceded_solution_id,
      dshs.superceding_solution_id AS superceding_solution_id,
      CASE
   ...
   FROM
      dim_solution_highest_supercedence dshs
      JOIN dim_solution ds ON ds.solution_id = dshs.superceding_solution_id
),
asset_vulnerability_count_by_solution AS (
   SELECT
      davs.asset_id,
      sss.superceding_solution_summary AS solution_summary,
      array_agg(ds.summary) AS superceded_solutions,
      dv.title AS vulnerabilities,
      (
         SELECT
            COALESCE(url, 'None')
         FROM
            dim_solution
         WHERE
            solution_id = MAX(sss.superceding_solution_id)
      ) AS url,
      (
         SELECT
            solution_type :: text
      ...
      (
         SELECT
            COUNT(DISTINCT (name, popularity))
         FROM
   ...
   FROM
      dim_asset_vulnerability_solution davs
   ...
   GROUP BY
      davs.asset_id,
      sss.superceding_solution_summary,
      dv.vulnerability_id,
      dv.title,
      dv.riskscore
   ORDER BY
      superceding_solution_summary
)
SELECT
   ds.name,
   da.ip_address,
   ...
   malware_kits
FROM
   asset_vulnerability_count_by_solution avcs
