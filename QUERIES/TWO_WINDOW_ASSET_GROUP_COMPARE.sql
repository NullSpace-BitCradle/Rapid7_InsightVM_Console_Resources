...
two_week_scans AS (
   ...
   ORDER BY
      site_change_totals_flattened.date DESC
),
last_scan AS (
   SELECT
      dag.asset_group_id,
      (
         SELECT
            tws.date
         FROM
            two_week_scans AS tws
         WHERE
            asset_group_id = dag.asset_group_id
         LIMIT
            1
      ) AS current_scan
   FROM
...
current_data AS (
   ...
   FROM
      two_week_scans tws
      JOIN last_scan ls ON ls.asset_group_id = tws.asset_group_id
      AND ls.current_scan = tws.date
),
prev_data AS (
   SELECT
      tws.name,
   ...
   FROM
      two_week_scans tws
      JOIN old_scan os ON os.asset_group_id = tws.asset_group_id
      AND os.prev_scan = tws.date
)
SELECT
   cd.name as asset_group,
   cd.date as current_date,
   cd.new_count as current_new_count,
   cd.old_count as current_old_count,
   cd.same_count as current_same_count,
   cd.total_vulnerabilities as current_total_vulns,
   cd.moderate_vulns AS current_moderate_vulns,
   cd.severe_vulns as current_severe_vulns,
   cd.critical_vulns AS current_critical_vulns,
   pd.date as previous_date,
   pd.new_count as previous_new_count,
   pd.old_count as previous_old_count,
   pd.same_count as previous_same_count,
   pd.total_vulnerabilities as previous_total_vulns,
   pd.moderate_vulns as previous_moderate_vulns,
   pd.severe_vulns AS previous_severe_vulns,
   pd.critical_vulns AS previous_critical_vulns,
   COALESCE(
      ROUND(
         CAST(
            (
               (
                  CAST(
                     (
                        cd.total_vulnerabilities - pd.total_vulnerabilities
                     ) AS DECIMAL
                  ) * 100
               ) / NULLIF(pd.total_vulnerabilities, 0)
            ) AS DECIMAL
         )
      ),
      0
   ) AS percent_change_total_vulns,
   COALESCE(
      ROUND(
         CAST(
            (
               (
                  CAST((cd.old_count - pd.old_count) AS DECIMAL) * 100
               ) / NULLIF(pd.old_count, 0)
            ) AS DECIMAL
         )
      ),
      0
   ) AS percent_change_old_vulns,
   COALESCE(
      ROUND(
         CAST(
            (
               (
                  CAST((cd.same_count - pd.same_count) AS DECIMAL) * 100
               ) / NULLIF(pd.same_count, 0)
            ) AS DECIMAL
         )
      ),
      0
   ) AS percent_change_same_vulns,
   COALESCE(
      ROUND(
         CAST(
            (
               CAST((cd.old_count) AS DECIMAL) * 100 / NULLIF(pd.total_vulnerabilities, 0)
            ) AS DECIMAL
         )
      ),
      0
   ) AS percent_vulns_fixed,
   CASE
      WHEN COALESCE(
         ROUND(
            CAST(
         ...
         0
      ) > 0 THEN '<font color=red>INCREASED</font>'
      WHEN COALESCE(
         ROUND(
            CAST(
               (
         ...
         0
      ) < 0 THEN '<font color=green>DECREASED</font>'
   ...
   CASE
      WHEN COALESCE(
         ...
         0
      ) > 0 THEN '<font color=green>INCREASED</font>'
      WHEN COALESCE(
         ROUND(
            CAST(
               (
         ...
         0
      ) < 0 THEN '<font color=red>DECREASED</font>'
      ELSE '<font color=blue>NO CHANGE</font>'
   ...
   CASE
      WHEN cd.total_vulnerabilities > pd.total_vulnerabilities THEN '<font color=red>INCREASED</font>'
