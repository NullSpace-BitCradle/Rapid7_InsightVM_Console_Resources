-- Purpose: New scan results from the past week only
-- Use Case: Weekly incremental updates, change detection, new vulnerability tracking
-- Data Sources: Rapid7 InsightVM Reporting Data Model
-- Grain: One row per vulnerability instance per asset per scan
-- Date Filter: scan.started >= CURRENT_DATE - INTERVAL '7 days'
-- Estimated Rows: 5K-50K (depends on weekly scan volume)

WITH vuln_categories AS (
	SELECT
		vulnerability_id,
		string_agg(DISTINCT category_name, ', ') AS vuln_category
	FROM dim_vulnerability_category
	GROUP BY vulnerability_id
),
asset_tags AS (
	SELECT
		ta.asset_id,
		string_agg(DISTINCT t.tag_name || ' (' || t.tag_type || ')', ', ') AS asset_tags
	FROM dim_tag_asset ta
	JOIN dim_tag t ON ta.tag_id = t.tag_id
	GROUP BY ta.asset_id
),
asset_groups AS (
	SELECT
		aga.asset_id,
		string_agg(DISTINCT ag.name, ', ') AS asset_groups
	FROM dim_asset_group_asset aga
	JOIN dim_asset_group ag ON aga.asset_group_id = ag.asset_group_id
	GROUP BY aga.asset_id
)
SELECT
	-- Scan information
	s.scan_id                                         AS "Scan_ID",
	to_char(s.started, 'YYYY-MM-DD')                  AS "Scan_Date",
	to_char(s.started, 'YYYY-MM-DD HH24:MI:SS')      AS "Scan_DateTime",
	s.scan_name                                       AS "Scan_Name",
	site.name                                         AS "Site_Name",
	s.finished                                        AS "Scan_Finished",

	-- Asset information
	a.asset_id                                        AS "Asset_ID",
	a.host_name                                       AS "Hostname",
	a.ip_address                                      AS "IP_Address",
	a.mac_address                                     AS "MAC_Address",
	os.name                                           AS "Operating_System",
	os.version                                        AS "OS_Version",
	os.architecture                                   AS "OS_Architecture",
	ht.description                                    AS "Host_Type",
	at.asset_tags                                     AS "Asset_Tags",
	ag.asset_groups                                   AS "Asset_Groups",

	-- Vulnerability identifiers and severity
	CASE
		WHEN v.title       LIKE '%CVE-%' THEN SUBSTRING(v.title       FROM 'CVE-[0-9]{4}-[0-9]{4,7}')
		WHEN v.description LIKE '%CVE-%' THEN SUBSTRING(v.description FROM 'CVE-[0-9]{4}-[0-9]{4,7}')
		ELSE NULL
	END                                               AS "CVE_ID",
	v.severity                                        AS "Severity",
	COALESCE(v.cvss_v3_score, v.cvss_v2_score, v.cvss_score, 0) AS "CVSS_Score",

	-- Service detail
	CASE WHEN i.port = -1 THEN NULL ELSE i.port END   AS "Service_Port",
	CASE WHEN i.service_id = -1 THEN NULL ELSE dsvc.name END AS "Service_Name",
	dp.name                                           AS "Service_Protocol",

	-- Vulnerability detail
	v.vulnerability_id                                AS "Vuln_ID",
	v.nexpose_id                                      AS "Vuln_Key",
	v.title                                           AS "Vuln_Title",
	vc.vuln_category                                  AS "Vuln_Category",
	htmlToText(v.description)                         AS "Description",
	htmlToText(i.proof)                               AS "Proof_Text",
	v.cvss_v3_vector                                  AS "CVSS_v3_Vector",
	v.exploits                                        AS "Exploits",
	v.malware_kits                                    AS "Malware_Kits",
	v.date_published                                  AS "Vuln_Publication_Date",

	-- Instance details
	i.key                                             AS "Instance_Key",
	i.status_id                                       AS "Vuln_Status",
	vs.description                                    AS "Status_Description",

	-- Best solution (asset+vuln specific when available)
	htmlToText(ds.summary)                            AS "Solution_Summary",
	htmlToText(ds.fix)                                AS "Solution_Fix",
	ds.solution_type                                  AS "Solution_Type",
	ds.estimate                                       AS "Solution_Estimate",
	ds.url                                            AS "Solution_URL"

FROM fact_asset_scan_vulnerability_instance        i
JOIN dim_asset                                a    ON a.asset_id = i.asset_id
JOIN dim_vulnerability                        v    ON v.vulnerability_id = i.vulnerability_id
JOIN dim_scan                                 s    ON s.scan_id = i.scan_id
JOIN dim_site_asset                           dsa  ON dsa.asset_id = a.asset_id
JOIN dim_site                                 site ON site.site_id = dsa.site_id
LEFT JOIN dim_operating_system                os   ON os.operating_system_id = a.operating_system_id
LEFT JOIN dim_host_type                       ht   ON ht.host_type_id = a.host_type_id
LEFT JOIN dim_protocol                        dp   ON dp.protocol_id = i.protocol_id
LEFT JOIN dim_service                         dsvc ON dsvc.service_id = i.service_id
LEFT JOIN dim_vulnerability_status            vs   ON vs.status_id = i.status_id
LEFT JOIN dim_asset_vulnerability_best_solution davbs
       ON davbs.asset_id = i.asset_id AND davbs.vulnerability_id = i.vulnerability_id
LEFT JOIN dim_solution                        ds   ON ds.solution_id = davbs.solution_id
LEFT JOIN vuln_categories                     vc   ON vc.vulnerability_id = v.vulnerability_id
LEFT JOIN asset_tags                           at   ON at.asset_id = a.asset_id
LEFT JOIN asset_groups                         ag   ON ag.asset_id = a.asset_id

WHERE s.started >= CURRENT_DATE - INTERVAL '7 days'
  AND COALESCE(v.cvss_v3_score, v.cvss_v2_score, v.cvss_score, 0) > 0

ORDER BY
	s.started DESC,
	COALESCE(v.cvss_v3_score, v.cvss_v2_score, v.cvss_score, 0) DESC,
	a.ip_address,
	v.title;
