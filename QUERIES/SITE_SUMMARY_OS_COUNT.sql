...
WITH assets_by_operating_system AS (
   SELECT
      site_id,
      SUM(
         CASE
   ...
   FROM
      (
         SELECT
            asset_id,
            LOWER(dos.description) LIKE '%windows%' AS windows,
            LOWER(dos.description) LIKE '%linux%' AS linux
         FROM
            dim_asset
...
SELECT
   ds.name,
   fs.assets,
   abos.windows_count AS windows_assets,
   abos.linux_count AS linux_assets,
   abos.other_count AS other_assets,
   fs.critical_vulnerabilities,
   fs.severe_vulnerabilities,
   fs.moderate_vulnerabilities,
   fs.riskscore
FROM
   fact_site fs
