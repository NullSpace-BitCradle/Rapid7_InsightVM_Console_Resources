...
SELECT
    ...
    lastScan(da.asset_id) AS last_scan_id,
    TO_CHAR(dscan.started, 'MM-DD-YYYY') as date,
    dsite.name AS site,
    dse.name AS scan_engine,
    da.ip_address,
    da.host_name,
    ds.name AS service_name,
    fass.port,
    dsf.name AS service_fingerprint,
    dsf.version
FROM
    fact_asset AS fa
...
WHERE
    lastScan(da.asset_id) = fas.scan_id
ORDER BY
    da.asset_id ASC,
    fass.port ASC