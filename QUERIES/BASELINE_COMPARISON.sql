...
asset_vuln_diff AS (
	SELECT
		CASE
			WHEN cavc.total_assets_now - pavc.total_assets_then > 0 THEN 'INCREASED'
			WHEN cavc.total_assets_now - pavc.total_assets_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS asset_diff,
		CASE
			WHEN cavc.total_vulns_now - pavc.total_vulns_then > 0 THEN 'INCREASED'
			WHEN cavc.total_vulns_now - pavc.total_vulns_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS vuln_diff,
		CASE
			WHEN cavc.total_vuln_risk_now - pavc.total_vuln_risk_then > 0 THEN 'INCREASED'
			WHEN cavc.total_vuln_risk_now - pavc.total_vuln_risk_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS vuln_risk_diff,
		CASE
			WHEN cavc.avg_risk_now - pavc.avg_risk_then > 0 THEN 'INCREASED'
			WHEN cavc.avg_risk_now - pavc.avg_risk_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS avg_risk_diff,
		CASE
			WHEN cavc.total_assets_now - pavc.total_assets_then > 0 THEN ROUND(
				(
					(
						CAST (
							(
		...
		CASE
			WHEN cavc.total_vulns_now - pavc.total_vulns_then > 0 THEN ROUND(
				(
					(
						CAST (
							(
		...
		END AS asset_diff_percent,
		CASE
			WHEN cavc.total_vuln_risk_now - pavc.total_vuln_risk_then > 0 THEN ROUND(
				CAST (
					(
						(
							CAST (
		...
		CASE
			WHEN cavc.avg_risk_now - pavc.avg_risk_then > 0 THEN ROUND(
				CAST (
					(
						(
							CAST (
...
vuln_severity_diff AS (
	SELECT
		CASE
			WHEN ccvc.critical_vulns_now - pcvc.critical_vulns_then > 0 THEN 'INCREASED'
			WHEN ccvc.critical_vulns_now - pcvc.critical_vulns_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS critical_diff,
		CASE
			WHEN csvc.severe_vulns_now - psvc.severe_vulns_then > 0 THEN 'INCREASED'
			WHEN csvc.severe_vulns_now - psvc.severe_vulns_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
		END AS severe_diff,
		CASE
			WHEN cmvc.moderate_vulns_now - pmvc.moderate_vulns_then > 0 THEN 'INCREASED'
			WHEN cmvc.moderate_vulns_now - pmvc.moderate_vulns_then < 0 THEN 'DECREASED'
			ELSE 'NO CHANGE'
